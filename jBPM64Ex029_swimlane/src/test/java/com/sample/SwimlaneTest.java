...
public class SwimlaneTest {
    ...
    public void testProcess() throws Exception {
        ...
        try {
            ...
            RuntimeManager manager = getRuntimeManager("sample-swimlane.bpmn");
            ...
            // ------------
            {
                ...
                for (TaskSummary taskSummary : list) {
                    System.out.println("john starts a task : taskId = " + taskSummary.getId() + ", status = " + taskSummary.getStatusId());
            ...
            
            {
                List<TaskSummary> list = taskService.getTasksAssignedAsPotentialOwner("john", "en-UK");
                for (TaskSummary taskSummary : list) {
                    // A task is assigned to Group "PM" but the task is automatically claimed to SwimlaneActorId. So the status is "Reserved"
                    System.out.println("john starts a task : taskId = " + taskSummary.getId() + ", status = " + taskSummary.getStatusId());
                    taskService.start(taskSummary.getId(), "john");
                    taskService.complete(taskSummary.getId(), "john", null);
                }
