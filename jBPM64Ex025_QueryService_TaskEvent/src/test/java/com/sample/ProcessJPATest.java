    ...
    public void testProcess() throws Exception {
        ...
        try {
            ...
            // -----------
            {
                ...
                ((QueryServiceImpl) queryService).setIdentityProvider(new TestIdentityProvider());
                ((QueryServiceImpl) queryService).setCommandService(new TransactionalCommandService(emf));
                ((QueryServiceImpl) queryService).init();

                SqlQueryDefinition query = new SqlQueryDefinition("getAllProcessInstance", "jdbc/jbpm-ds");
                query.setExpression("select te.* " + "from TaskEvent te ");

                queryService.registerQuery(query);
                List<TaskEvent> teList = queryService.query(query.getName(), TaskEventQueryMapper.get(), new QueryContext());
                for (TaskEvent te : teList) {
                    System.out.println("taskId = " + te.getTaskId() + ", type = " + te.getType() + ", userId = " + te.getUserId());
                }
