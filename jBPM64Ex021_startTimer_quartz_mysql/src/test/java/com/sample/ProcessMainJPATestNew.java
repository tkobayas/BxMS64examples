 ...
 */
public class ProcessMainJPATestNew extends TestCase {
    ...
    public void testTimer() throws Exception {
        ...
        // ---- 1st boot
        {
            RuntimeManager manager = getRuntimeManager("starttimer-process.bpmn2");
...
//            ProcessInstance pi = ksession.startProcess("starttimer-process", null);
//            System.out.println(new Date() + " : " + "A process instance started : pid = " + pi.getId());
//            piid = pi.getId();

            manager.disposeRuntimeEngine(runtime);
            System.out.println(new Date() + " : " + "RuntimeEngine disposed");

            Thread.sleep(30000); // make sure the timer is fired at least once (timer's duration + org.quartz.jobStore.misfireThreshold)
    ...
    private static void cleanupQuartzTables() {
        Scanner scanner;
        if (mySQL) {
            scanner = new Scanner(ProcessMainJPATestNew.class.getResourceAsStream("/tables_mysql.sql")).useDelimiter(";");
        } else {
            scanner = new Scanner(ProcessMainJPATestNew.class.getResourceAsStream("/tables_h2.sql")).useDelimiter(";");
        }
